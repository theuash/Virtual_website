...
import {
  Wallet, ArrowUpRight, ArrowDownLeft, Loader2, X,
...
function WithdrawModal({ balance, onClose, onSuccess }) {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      await api.post('/supervisor/wallet/withdraw', { amount: amt, method, accountDetails: details });
      setDone(true);
      ...
      <motion.div
        ...
        {done ? (
          ...
          <div className="p-6 space-y-5">
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                {QUICK_AMOUNTS.filter(a => a <= balance).map(a => (
                  <button key={a} onClick={() => setAmount(String(a))}
                    ...
                    style={{
                      borderColor: amount === String(a) ? '#f59e0b' : 'var(--border)',
                ...
                {balance > 0 && (
                  <button onClick={() => setAmount(String(balance))}
                    className="px-3 py-1 rounded-full text-xs font-semibold border transition-all"
                    style={{
                      borderColor: amount === String(balance) ? '#f59e0b' : 'var(--border)',
                      color:       amount === String(balance) ? '#f59e0b' : 'var(--text-secondary)',
                      background:  'var(--bg-card)',
                    }}>
                    All
                  </button>
            ...
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest mb-2 block" style={{ color: 'var(--text-secondary)' }}>Withdrawal Method</label>
              <div className="space-y-2">
                {WITHDRAW_METHODS.map(m => (
                  <button key={m.id} onClick={() => setMethod(m.id)}
                    className="w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left"
                    style={{
                      background:  method === m.id ? '#f59e0b' : 'var(--bg-card)',
                      borderColor: method === m.id ? '#f59e0b' : 'var(--border)',
...
export default function SupervisorWallet() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get('/supervisor/wallet');
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                { label: 'Total Earned',      value: wallet?.totalEarned ?? 0,    color: '#f59e0b', icon: <TrendingUp size={18} strokeWidth={1.5} /> },
            ...
            <button onClick={() => setShowWithdraw(true)} disabled={(wallet?.balance ?? 0) < 100}
                ...
                <div className="flex gap-1.5">
                  {['all', 'earning', 'withdrawal'].map(f => (
                    <button key={f} onClick={() => setTxFilter(f)}
                      ...
                      style={{
                        background:  txFilter === f ? '#f59e0b' : 'var(--bg-card)',
                        color:       txFilter === f ? '#fff' : 'var(--text-secondary)',
                        borderColor: txFilter === f ? '#f59e0b' : 'var(--border)',
                      }}>
                      {f}
                    </button>
                  ))}
              ...
              {sorted.length === 0 ? (
                ...
                <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
                  {sorted.map((tx, i) => {
                    ...
                    return (
                      <motion.div key={tx._id || i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="flex items-center gap-0.5 text-[9px] font-bold" style={{ color: statusStyle.color }}>
